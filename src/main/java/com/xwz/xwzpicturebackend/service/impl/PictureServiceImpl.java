...
import com.xwz.xwzpicturebackend.api.aliyunai.model.CreateOutPaintingTaskRequest;
...
import com.xwz.xwzpicturebackend.utils.ColorSimilarUtils;
import com.xwz.xwzpicturebackend.utils.ColorTransformUtils;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        // 1.2 空间校验
        Long spaceId = verifySpace(pictureUploadRequest, loginUser);
        // 2. 新增还是更新图片
        // 2.1 用于判断是新增还是更新图片
        Long pictureId = null;
        ...
        // 2.1.1 如果是更新图片，需要校验图片是否存在
        if (pictureId != null) {
            Picture oldPicture = this.getById(pictureId);
            ThrowUtils.throwIf(oldPicture == null, ErrorCode.NOT_FOUND_ERROR, "图片不存在");
            // 仅本人或管理员可编辑 Change ：改为使用统一的权限校验
...
//            }
        ...
        // 2.2.2 执行模版方法，上传图片到COS
        UploadPictureResult uploadPictureResult = pictureUploadTemplate.uploadPicture(inputSource, uploadPathPrefix);
        // 3 入库
        // 3.1 构造要入库的图片信息
        Picture picture = structurePicture(loginUser, uploadPictureResult, spaceId, pictureId);
        if (pictureId != null) {
            // 如果是更新，需要补充 id 和编辑时间
    ...
    private Picture structurePicture(User loginUser, UploadPictureResult uploadPictureResult, Long spaceId, Long pictureId) {
    ...
    private Long verifySpace(PictureUploadRequest pictureUploadRequest, User loginUser) {
        // 空间权限校验
...
//            }
    ...
    public QueryWrapper<Picture> getQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        // 如下这个条件就相当于，为nullSpaceId=true的话就会拼接 and nullSpaceId = null这个条件，如果没有的话就走上一条具体的查询
    ...
    /**
     * 管理人审核 用户上传图片 ：
     * 逻辑很简单，就是根据新传入的图片审核信息，来对旧数据进行更新的一个方法
     * @param pictureReviewRequest 图片审核请求实体
